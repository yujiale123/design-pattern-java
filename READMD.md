...
工厂模式也被称为工厂方法模式，是一种创造型模式，其在父类中提供一个创建对象的方法，允许子类决定实例化对象的类型。
主要意图是定义一个创建对象的接口，让其子类决定实例化哪一个工厂类，工厂模式使其创建过程延迟到子类进行。

为了提供代码结构的扩展性，屏蔽每⼀个功能类中的具体实现逻辑。

工厂模式分为简单工厂模式和工厂方法模式

- 简单工厂适用于工厂类负责创建的对象**较少**的场景，且客户端只需要传入工厂类的参数，对于如何创建对象的逻辑不需要关心。
  简单工厂违背了开闭原则，同时对于工厂的扩展不易。
